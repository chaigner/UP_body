...
% usage: evalAllDatasets(pulseType, wvfrms, numTailored, numkTpoints, 
%                     c_diffrand, c_lambdaexp, lambdavec, c_kTpoints, prbp)
...
%                   c_diffrand  = index of randomized init
%                   c_lambdaexp = index of regularization parameter
%                   lambdavec   = array of regularization parameters
%                   c_kTpoints  = index of kT point 
%                   prbp        = struct with most problem related params
% pulseType can be 'default', 'tailored' or 'UP' and wvfrms must contain 
% the variables wvfrms.k and wvfrms.rf. 
% 
% Created by Christoph S. Aigner, PTB, June 2021.
% Email: christoph.aigner@ptb.de

function evalAllDatasets(pulseType, wvfrms, numTailored, numkTpoints, c_diffrand, c_lambdaexp, lambdavec, c_kTpoints, prbp)

    %initialize the arrays for the first time
    if ~(exist('CV_pre_all_subectrob','var'))
        CV_post_all_subectrob = zeros(max(numkTpoints),length(prbp.allmaps));
    %     farmse_subectrob = zeros(max(numkTpoints),length(prbp.allmaps));
    end
    ...
    CV_post_all_subectrob(c_kTpoints,:) = zeros(1,length(prbp.allmaps));
    % farmse_subectrob(c_kTpoints,:)      = zeros(1,length(prbp.allmaps));

    for c_subj=1:(length(prbp.allmaps)) %loop over the B1+ datasets
        ...
        % get the maps from prbp.allmaps
        maps.b1   = prbp.allmaps{c_subj}.b1;
        maps.mask = prbp.allmaps{c_subj}.mask;
        maps.b0   = prbp.allmaps{c_subj}.b0;
        ...
        % define the grid
        xg = -prbp.fov(1)/2:prbp.fov(1)/dimxyz(1):prbp.fov(1)/2-prbp.fov(1)/dimxyz(1);
        yg = -prbp.fov(2)/2:prbp.fov(2)/dimxyz(2):prbp.fov(2)/2-prbp.fov(2)/dimxyz(2);
        zg = -prbp.fov(3)/2:prbp.fov(3)/dimxyz(3):prbp.fov(3)/2-prbp.fov(3)/dimxyz(3);
        [xx,yy,zz] = ndgrid(xg, yg, zg);
        xx = [xx(:) yy(:) zz(:)];

        % in case its not already done, subtract off first coil's phase
        sens = maps.b1.*exp(-1i*repmat(angle(maps.b1(:,:,:,1)),[1 1 1 prbp.Nc]));

        % reshape B1 maps and initialize system matrix A
        Ns   = size(sens,1)*size(sens,2)*size(sens,3); % number of voxels
        sensd = reshape(sens,[Ns prbp.Nc]);                 
        A = zeros(Ns,prbp.Nc*Npulset);
        ...
        subplot(ceil(length(prbp.allmaps)/5)+1,5,c_pos+4)
    ...
    elseif strcmp(pulseType,'UP')
            name = [pulseType,num2str(prbp.Nm),'-',num2str(c_kTpoints),'kT, phaseinit=',num2str(c_diffrand), ', regularization=',num2str(lambdavec(c_lambdaexp))];
    end
