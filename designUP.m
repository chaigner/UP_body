...
% 
% usage: wvfrms = designUP(pulseType, numkTpoints, numPhaseInit, 
...
function wvfrms = designUP(pulseType, numkTpoints, numPhaseInit, lambdavec, phsinitmode, b_evalAllDatasets, numTailored, prbp) 
    ...
    for c_kTpoints = numkTpoints
        for c_diffrand = numPhaseInit
            for c_lambdaexp=1:length(lambdavec)
                ...
                Nm = length(prbp.librarymaps); %number of B1+datasets in the library
                prbp.Nm = Nm;

                % initialize maps (to be sure to have the right size)
                maps.numberofmaps = Nm;
                maps.b1   = zeros(prbp.B1Dim(1),prbp.B1Dim(2),prbp.B1Dim(3)*Nm,prbp.Nc);
                maps.mask = zeros(prbp.B1Dim(2),prbp.B1Dim(2),prbp.B1Dim(3)*Nm);
                maps.b0   = zeros(prbp.B1Dim(2),prbp.B1Dim(2),prbp.B1Dim(3)*Nm);
                maps.fov = prbp.fov;

                % get the maps from allmaps
                for c_dat=1:Nm
                    maps.b1(:,:,(1:prbp.B1Dim(3))+(c_dat-1)*prbp.B1Dim(3),:) = prbp.librarymaps{c_dat}.b1;
                    maps.mask(:,:,(1:prbp.B1Dim(3))+(c_dat-1)*prbp.B1Dim(3)) = prbp.librarymaps{c_dat}.mask;
                    maps.b0(:,:,(1:prbp.B1Dim(3))+(c_dat-1)*prbp.B1Dim(3))   = prbp.librarymaps{c_dat}.mask*0;
                end
